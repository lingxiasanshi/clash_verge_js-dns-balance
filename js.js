...
function main(config) {
  ...
  if (config.proxies && config.proxies.length > 0) {
    const proxies = config.proxies.map(x => x.name);
    
    // 均衡负载-散列
    const loadBalanceHash = {
      ...groupBaseOption,
      name: "均衡负载-散列",
      type: "load-balance",
      proxies: proxies,
    ...
    // 将新代理组添加到所有现有代理组的可选列表中（置于最前）
    config["proxy-groups"].forEach(group => {
      if (group.proxies) {
        group.proxies.unshift(
          autoSelect.name,
          loadBalanceRoundRobin.name,
          loadBalanceHash.name
        );
      }
    });

    // 添加新代理组到配置中
    config["proxy-groups"].push(loadBalanceHash, loadBalanceRoundRobin, autoSelect);
  }
